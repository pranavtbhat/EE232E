...
file_names = list.files("data/gplus/")
file_ids = sub("^([^.]*).*", "\\1", file_names)
ego_node_ids = unique(file_ids) 
print(ego_node_ids)
cat("Total Number of Ego Nodes = \n", length(ego_node_ids))

ids_circles = numeric()
for (id in ego_node_ids) {
    # get the number of circles
    circles_file = paste("data/gplus/" , id , ".circles" , sep="")
...
cat("Total IDs with > 2 circles = \n", length(ids_circles))

for (id in  c(ids_circles[2], ids_circles[5], ids_circles[12])) {

    edges_file = paste("data/gplus/" , id  , ".edges" , sep="") # edge list
    circles_file = paste("data/gplus/" , id , ".circles" , sep="") # circles list
    ...
    corrplot(c_percentage, method="color", cl.lim=c(0,1), tl.cex = 2, tl.col = 'black', col= colorRampPalette(c("white","pink", "red"))(10))
